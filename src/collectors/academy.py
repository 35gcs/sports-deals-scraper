"""Academy Sports + Outdoors collector."""
...
class AcademyCollector(BaseCollector):
    """Collector for Academy Sports + Outdoors deals."""
    
    def _parse_item_specific(self, item_html: str, deal_data: Dict[str, Any]) -> Dict[str, Any]:
        """Parse Academy-specific data."""
        ...
        item_elem = parser.css_first('[data-product-id], [data-sku]')
        if item_elem:
            product_id = item_elem.attributes.get('data-product-id') or item_elem.attributes.get('data-sku')
            if product_id:
        ...
        breadcrumb_elem = parser.css_first('.breadcrumb, .nav-breadcrumb, .category-path')
        if breadcrumb_elem:
            breadcrumb_text = breadcrumb_elem.text()
            deal_data['category'] = self._map_category(breadcrumb_text)
            deal_data['sport'] = self._map_sport(breadcrumb_text)
        
        # Extract size information
        size_container = parser.css_first('.size-selector, .size-options, .available-sizes')
        if size_container:
        ...
        stock_elem = parser.css_first('.stock-status, .availability, .inventory-status')
        if stock_elem:
        ...
        promo_elem = parser.css_first('.sale-badge, .clearance-badge, .promo-badge, .deal-badge')
        if promo_elem:
        ...
        coupon_elem = parser.css_first('.coupon-info, .promo-code, .discount-code')
        if coupon_elem:
        ...
        # Enhanced youth detection for Academy
        ...
        age_elem = parser.css_first('.age-range, .recommended-age')
        if age_elem:
    ...
    def _map_category(self, category_text: str) -> str:
        """Map Academy category text to our category enum."""
        ...
        elif any(word in category_text for word in ['helmet', 'pad', 'guard', 'protection', 'safety']):
    ...
    def _map_sport(self, category_text: str) -> str:
        """Map Academy category text to our sport enum."""
        category_text = category_text.lower()
        ...
        elif any(word in category_text for word in ['running', 'jog', 'athletic']):
    ...
    async def collect_deals(self) -> List[Deal]:
        """Collect deals from Academy Sports + Outdoors."""
