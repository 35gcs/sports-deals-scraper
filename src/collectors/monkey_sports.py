"""Monkey Sports collector (HockeyMonkey, BaseballMonkey, LacrosseMonkey)."""
...
class MonkeySportsCollector(BaseCollector):
    """Collector for Monkey Sports deals (HockeyMonkey, BaseballMonkey, LacrosseMonkey)."""
    
    def _parse_item_specific(self, item_html: str, deal_data: Dict[str, Any]) -> Dict[str, Any]:
        """Parse Monkey Sports-specific data."""
        ...
        # Determine sport based on retailer name
        retailer_name = self.config.name.lower()
        if 'hockey' in retailer_name:
            deal_data['sport'] = Sport.HOCKEY.value
        elif 'baseball' in retailer_name:
            deal_data['sport'] = Sport.BASEBALL.value
        elif 'lacrosse' in retailer_name:
            deal_data['sport'] = Sport.LACROSSE.value
        ...
        # Enhanced youth detection for Monkey Sports
    ...
    def _map_category(self, category_text: str) -> str:
        """Map Monkey Sports category text to our category enum."""
        category_text = category_text.lower()
        
        if any(word in category_text for word in ['shoe', 'cleat', 'sneaker', 'boot', 'footwear', 'skate']):
        ...
        elif any(word in category_text for word in ['helmet', 'pad', 'guard', 'protection', 'safety', 'glove']):
            return Category.PROTECTIVE.value
        elif any(word in category_text for word in ['ball', 'bat', 'stick', 'racket', 'club', 'equipment', 'puck']):
    ...
    def _map_sport(self, category_text: str) -> str:
        """Map Monkey Sports category text to our sport enum."""
        # Sport is determined by retailer name
        retailer_name = self.config.name.lower()
        if 'hockey' in retailer_name:
    ...
    async def collect_deals(self) -> List[Deal]:
        """Collect deals from Monkey Sports."""
