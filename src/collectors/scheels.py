...
class ScheelsCollector(BaseCollector):
    """Collector for Scheels deals."""
    
    def _parse_item_specific(self, item_html: str, deal_data: Dict[str, Any]) -> Dict[str, Any]:
        """Parse Scheels-specific data."""
        from selectolax.parser import HTMLParser
        parser = HTMLParser(item_html)
        
        # Extract SKU/Product ID
        sku_elem = parser.css_first('.product-sku, .sku, [data-product-sku]')
        if sku_elem:
        ...
        # Extract brand
        brand_elem = parser.css_first('.brand-name, .product-brand, .manufacturer-name')
        if brand_elem:
        ...
        category_elem = parser.css_first('.breadcrumb, .category-navigation, .product-category')
        if category_elem:
        ...
        if size_container:
            size_elements = size_container.css('a, button, .size-item')
            sizes = []
        ...
        # Enhanced youth detection for Scheels
    ...
    def _map_category(self, category_text: str) -> str:
        """Map Scheels category text to our category enum."""
    ...
    def _map_sport(self, category_text: str) -> str:
        """Map Scheels category text to our sport enum."""
        category_text = category_text.lower()
